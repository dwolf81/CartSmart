-- Updates f_best_deals to:
-- 1) accept optional p_store_id for store-scoped best deals
-- 2) dynamically pick the best deal per product (no product.deal_id dependency)
--
-- Uses DISTINCT ON for a simple “top 1 per product” selection.
...
begin
  ...
  where
    ...
    -- If no product type filter, return only products that actually have a deal.
