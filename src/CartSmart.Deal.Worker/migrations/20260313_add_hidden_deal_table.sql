-- Allow users to hide deals they aren't interested in.
-- Hidden deals are filtered out of f_get_product_deals_2 when a user_id is supplied.

CREATE TABLE IF NOT EXISTS hidden_deal (
...
CREATE INDEX IF NOT EXISTS idx_hidden_deal_user_deal ON hidden_deal (user_id, deal_id);

-- Enable RLS
ALTER TABLE hidden_deal ENABLE ROW LEVEL SECURITY;

-- Users can read/write their own rows; service role bypasses RLS.
CREATE POLICY hidden_deal_user_policy ON hidden_deal
  FOR ALL
  USING (auth.uid()::text = user_id::text)
  WITH CHECK (auth.uid()::text = user_id::text);

-- Update f_get_product_deals_2 to exclude hidden deals for the current user.
