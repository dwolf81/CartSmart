-- Creates a helper function to upsert derived deal_product rows for store-wide coupon/external deals
-- based on an approved (or admin-submitted) direct deal.
--
-- Rules:
-- - Only runs when the direct deal is approved (deal_status_id = 2) OR the submitter is an admin.
-- - Creates/updates deal_product rows for each store-wide deal on the same store (deal_type_id IN (2,4)).

-- Ensure the linkage column exists (used to sync/cleanup derived store-wide rows)
