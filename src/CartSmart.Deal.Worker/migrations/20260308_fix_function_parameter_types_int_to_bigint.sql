-- Fix: change function parameter types from int to bigint to match the deal.id column type.
-- The trigger f_sync_stacked_deal_products_from_deal() passes NEW.id (bigint) into these
-- functions, but they were defined with int parameters, causing:
--   ERROR 42883: function public.f_upsert_storewide_deal_products_for_direct_deal(bigint) does not exist

-- Drop the old int-signature overloads so we don't leave stale copies behind.
DROP FUNCTION IF EXISTS public.f_upsert_storewide_deal_products_for_direct_deal(int);
DROP FUNCTION IF EXISTS public.f_upsert_stacked_deal_products_for_store(int, int);

-- 1) Recreate f_upsert_storewide_deal_products_for_direct_deal with bigint parameter
...
-- 2) Recreate f_upsert_stacked_deal_products_for_store with bigint parameters
